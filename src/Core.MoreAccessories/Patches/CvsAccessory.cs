        ...
        public static class CvsAccessory_Patches
        {
            ...
            private static class CvsAccessory_CalculateUI_Patches
            {
                private static bool Prefix(CvsAccessory __instance)
                {
                    ...
                    var part = _self.GetPart((int)__instance.slotNo);
                    ...
                    var tglAcsGroup = __instance.tglAcsGroup;
                    for (var i = 0; i < tglAcsGroup.Length; i++)
                    {
            ...
            private static class CvsAccessory_UpdateCustomUI_Patches
            {
                private static bool Prefix(CvsAccessory __instance)
                {
...
#endif
                    var num = _self.GetPart(selectIndex).type - 120;
                    ...
                    var cvsColor = __instance.cvsColor;
                    for (var i = 0; i < _self.GetPartsLength(); i++)
                    {
                        var part = _self.GetPart(i);
                        for (var j = 0; j < 4; j++)
                        {
            ...
            private static class CvsAccessory_UpdateSlotName_Patches
            {
                private static bool Prefix(CvsAccessory __instance)
                {
                    var textSlotName = __instance.textSlotName;
                    ...
                    if (_self.GetPartsLength() <= index || _self.GetPart(index).type == 120)
                    {
                        textSlotName.text = _self._isParty ? $"Slot {index + 1:00}" : $"スロット{index + 1:00}";
                    }
                    else
                    {
                        if (index < 20)
                        ...
                        {
                            if (_self._charaMakerData.infoAccessory[index - 20] != null)
                                textSlotName.text = _self._charaMakerData.infoAccessory[index - 20].Name;
                        }
                    }
                    return false;
                }
            ...
            private static class CvsAccessory_UpdateSelectAccessoryKind_Patches
            {
                ...
                {
                    var textAcsKind = __instance.textAcsKind;
                    if (textAcsKind)
                    {
                        textAcsKind.text = name;
                    }
                    var imgAcsKind = __instance.imgAcsKind;
                    if (imgAcsKind)
                    {
                    ...
                    if (part.id != index)
                    {
                        ...
                        var categoryInfo = CustomBase.Instance.chaCtrl.lstCtrl.GetCategoryInfo((ChaListDefine.CategoryNo)part.type);
                        ListInfoBase listInfoBase = null;
                        categoryInfo.TryGetValue(part.id, out listInfoBase);
                        var flag = listInfoBase != null && 1 == listInfoBase.GetInfoInt(ChaListDefine.KeyType.HideHair);
            ...
            private static class CvsAccessory_UpdateSelectAccessoryParent_Patches
            {
                private static bool Prefix(CvsAccessory __instance, int index)
                {
                    var textAcsParent = __instance.textAcsParent;
                    if (textAcsParent)
                    {
                    ...
                    var array = (from key in Enum.GetNames(typeof(ChaAccessoryDefine.AccessoryParentKey))
                                 where key != "none"
                                 select key).ToArray();
            ...
            private static class CvsAccessory_UpdateAccessoryParentInfo_Patches
            {
                private static bool Prefix(CvsAccessory __instance)
                {
                    ...
                    if (0 <= num)
                    {
                        ...
                        {
                            var num2 = __instance.cusAcsParentWin.UpdateCustomUI();
            ...
            private static class CvsAccessory_FuncUpdateAcsParent_Patches
            {
                private static bool Prefix(CvsAccessory __instance, bool history, ref bool __result)
                {
                    if (history)
                    {
                        for (var i = 0; i < 20; i++)
                        {
                            CustomBase.Instance.chaCtrl.ChangeAccessoryParent(i, _self.GetPart(i).parentKey);
                        }
                        __result = true;
                        return false;
                    }
                    __result = CustomBase.Instance.chaCtrl.ChangeAccessoryParent((int)__instance.slotNo, _self.GetPart((int)__instance.slotNo).parentKey);
                    return false;
                }
            ...
            private static class CvsAccessory_UpdateAcsColor01_Patches
            {
                ...
                {
                    _self.GetPart((int)__instance.slotNo).color[0] = color;
                    if ((int)__instance.slotNo < 20)
            ...
            private static class CvsAccessory_UpdateAcsColor02_Patches
            {
                private static bool Prefix(CvsAccessory __instance, Color color)
                {
                    _self.GetPart((int)__instance.slotNo).color[1] = color;
                    if ((int)__instance.slotNo < 20)
            ...
            private static class CvsAccessory_UpdateAcsColor03_Patches
            {
                private static bool Prefix(CvsAccessory __instance, Color color)
                {
                    _self.GetPart((int)__instance.slotNo).color[2] = color;
                    if ((int)__instance.slotNo < 20)
            ...
            private static class CvsAccessory_UpdateAcsColor04_Patches
            {
                private static bool Prefix(CvsAccessory __instance, Color color)
                {
                    _self.GetPart((int)__instance.slotNo).color[3] = color;
                    if ((int)__instance.slotNo < 20)
            ...
            private static class CvsAccessory_FuncUpdateAcsPosAdd_Patches
            {
                private static bool Prefix(CvsAccessory __instance, int correctNo, int xyz, bool add, float val)
                {
                    ...
                    if ((int)__instance.slotNo < 20)
                        CustomBase.Instance.chaCtrl.chaFile.GetCoordinate(CustomBase.Instance.chaCtrl.chaFile.status.GetCoordinateType()).accessory.parts[(int)__instance.slotNo].addMove[correctNo, 0] = _self.GetPart((int)__instance.slotNo).addMove[correctNo, 0];
                    return false;
                }
            ...
            private static class CvsAccessory_FuncUpdateAcsRotAdd_Patches
            {
                private static bool Prefix(CvsAccessory __instance, int correctNo, int xyz, bool add, float val)
                {
                    ...
                    if ((int)__instance.slotNo < 20)
                        CustomBase.Instance.chaCtrl.chaFile.GetCoordinate(CustomBase.Instance.chaCtrl.chaFile.status.GetCoordinateType()).accessory.parts[(int)__instance.slotNo].addMove[correctNo, 1] = _self.GetPart((int)__instance.slotNo).addMove[correctNo, 1];
                    return false;
                }
            ...
            private static class CvsAccessory_FuncUpdateAcsSclAdd_Patches
            {
                private static bool Prefix(CvsAccessory __instance, int correctNo, int xyz, bool add, float val)
                {
                    ...
                    if ((int)__instance.slotNo < 20)
                        CustomBase.Instance.chaCtrl.chaFile.GetCoordinate(CustomBase.Instance.chaCtrl.chaFile.status.GetCoordinateType()).accessory.parts[(int)__instance.slotNo].addMove[correctNo, 2] = _self.GetPart((int)__instance.slotNo).addMove[correctNo, 2];
                    return false;
                }
            ...
            private static class CvsAccessory_FuncUpdateAcsMovePaste_Patches
            {
                private static bool Prefix(CvsAccessory __instance, int correctNo, Vector3[] value)
                {
                    ...
                    {
                        var part = _self.GetPart((int)__instance.slotNo);
                        var setPart = CustomBase.Instance.chaCtrl.chaFile.GetCoordinate(CustomBase.Instance.chaCtrl.chaFile.status.GetCoordinateType()).accessory.parts[(int)__instance.slotNo];
            ...
            private static class CvsAccessory_ChangeParentAndMoveSettingVisible_Patches
            {
                ...
                {
                    var part = _self.GetPart((int)__instance.slotNo);
                    var flag = (__instance.ddAcsType).value != 0;
                    var categoryInfo = CustomBase.Instance.chaCtrl.lstCtrl.GetCategoryInfo((ChaListDefine.CategoryNo)part.type);
                    ListInfoBase listInfoBase = null;
                    categoryInfo.TryGetValue(part.id, out listInfoBase);
                    ...
                    if ((int)__instance.slotNo < 20)
                        ...
                        objAcsMove = _self._charaMakerData.objAcsMove[(int)__instance.slotNo - 20][1];
                ...
                private static bool Prefix(CvsAccessory __instance)
                {
                    ...
                    if ((__instance.ddAcsType).value != 0)
                    {
                        ...
                        if ((int)__instance.slotNo < 20)
                            ...
                            cmp = _self._accessoriesByChar[CustomBase.Instance.chaCtrl.chaFile].cusAcsCmp[(int)__instance.slotNo - 20];
                        if (cmp != null)
                        {
                            if (cmp.useColor01)
                            {
            ...
            private static class CvsAccessory_SetDefaultColorWindow_Patches
            {
                ...
                {
                    __instance.cvsColor.Setup($"スロット{no + 1:00} カラー①", (CvsColor.ConnectColorKind)(no * 4 + 124), _self.GetPart(no).color[0], __instance.UpdateAcsColor01, __instance.UpdateAcsColorHistory(), false);
                    return false;
                }
            ...
            private static class CvsAccessory_SetControllerTransform_Patches
            {
                private static bool Prefix(CvsAccessory __instance, int guidNo)
                {
                    ...
                    if (index < 20)
                        ...
                        gameObject = _self._charaMakerData.objAcsMove[index - 20][guidNo];
                    if (null == gameObject)
                    {
                        return false;
                    }
                ...
                private static bool Prefix(CvsAccessory __instance, int guidNo, bool updateInfo)
                {
                    ...
                    if (array[0].isOn)
                    {
                        ...
                        if (updateInfo)
                        {
                            ...
                            if ((int)__instance.slotNo < 20)
                                CustomBase.Instance.chaCtrl.chaFile.GetCoordinate(CustomBase.Instance.chaCtrl.chaFile.status.GetCoordinateType()).accessory.parts[(int)__instance.slotNo].addMove[guidNo, 0] = _self.GetPart((int)__instance.slotNo).addMove[guidNo, 0];
                    ...
                    else
                    {
                        ...
                        if (updateInfo)
                        {
                            ...
                            if ((int)__instance.slotNo < 20)
                                CustomBase.Instance.chaCtrl.chaFile.GetCoordinate(CustomBase.Instance.chaCtrl.chaFile.status.GetCoordinateType()).accessory.parts[(int)__instance.slotNo].addMove[guidNo, 1] = _self.GetPart((int)__instance.slotNo).addMove[guidNo, 1];
            ...
            private static class CvsAccessory_UpdateDrawControllerState_Patches
            {
                private static bool Prefix(CvsAccessory __instance, int guidNo)
                {
                    ...
                    var toggle = guidNo != 0 ? __instance.tglDrawController02 : __instance.tglDrawController01;
                    Toggle[] array =
                    {
                    guidNo != 0 ? (__instance.tglControllerType02)[0] : (__instance.tglControllerType01)[0],
                    guidNo != 0 ? (__instance.tglControllerType02)[1] : (__instance.tglControllerType01)[1]
                };
                    var slider = guidNo != 0 ? __instance.sldControllerSpeed02 : __instance.sldControllerSpeed01;
                    var slider2 = guidNo != 0 ? __instance.sldControllerScale02 : __instance.sldControllerScale01;
            ...
            private static class CvsAccessory_Start_Patches
            {
                ...
                private static async UniTask Test(CvsAccessory __instance)
                {
                    ...
                    var nSlotNo = (int)__instance.slotNo;
                    if (nSlotNo < 20)
                    {
                    ...
                    {
                        var accessoryDefaultParentStr = CustomBase.Instance.chaCtrl.GetAccessoryDefaultParentStr((int)__instance.slotNo);
                        _self.GetPart((int)__instance.slotNo).parentKey = accessoryDefaultParentStr;
                        if ((int)__instance.slotNo < 20)
                    ...
                    __instance.btnAcsColor01.OnClickAsObservable().Subscribe(delegate
                    {
                        ...
                        {
                            __instance.cvsColor.Setup($"スロット{(int)__instance.slotNo + 1:00} カラー①", (CvsColor.ConnectColorKind)((int)__instance.slotNo * 4 + 124), _self.GetPart((int)__instance.slotNo).color[0], __instance.UpdateAcsColor01, __instance.UpdateAcsColorHistory(), false);
                        }
                    });
                    __instance.btnAcsColor02.OnClickAsObservable().Subscribe(delegate
                    {
                        ...
                        {
                            __instance.cvsColor.Setup($"スロット{(int)__instance.slotNo + 1:00} カラー②", (CvsColor.ConnectColorKind)((int)__instance.slotNo * 4 + 124 + 1), _self.GetPart((int)__instance.slotNo).color[1], __instance.UpdateAcsColor02, __instance.UpdateAcsColorHistory(), false);
                        }
                    });
                    __instance.btnAcsColor03.OnClickAsObservable().Subscribe(delegate
                    {
                        ...
                        {
                            __instance.cvsColor.Setup($"スロット{(int)__instance.slotNo + 1:00} カラー③", (CvsColor.ConnectColorKind)((int)__instance.slotNo * 4 + 124 + 2), _self.GetPart((int)__instance.slotNo).color[2], __instance.UpdateAcsColor03, __instance.UpdateAcsColorHistory(), false);
                        }
                    });
                    __instance.btnAcsColor04.OnClickAsObservable().Subscribe(delegate
                    {
                        ...
                        {
                            __instance.cvsColor.Setup($"スロット{(int)__instance.slotNo + 1:00} カラー④", (CvsColor.ConnectColorKind)((int)__instance.slotNo * 4 + 124 + 3), _self.GetPart((int)__instance.slotNo).color[3], __instance.UpdateAcsColor04, __instance.UpdateAcsColorHistory(), true);
                        }
                    });
                     ...
                     select item).ToList().ForEach(item =>
                     {
                        ...
                        (from item in (__instance.tglHideTiming).Select((Toggle tgl, int idx) => new { tgl, idx })
                         ...
                         select item).ToList().ForEach(item =>
                         {
                             ...
                             {
                                 ChaFileAccessory.PartsInfo part = _self.GetPart((int)__instance.slotNo);
            ...
            private static class CvsAccessoryChange_CalculateUI_Patches
            {
                private static void Postfix(CvsAccessoryChange __instance)
                {
                    if (_self._charaMakerData.nowAccessories.Count > _self._additionalCharaMakerSlots.Count)
                        return;
                    for (var i = 0; i < _self._charaMakerData.nowAccessories.Count; i++)
                    {
                        var slot = _self._additionalCharaMakerSlots[i];
                        if (slot.transferSlotObject.activeSelf)
                        {
                            var part = _self._charaMakerData.nowAccessories[i];
                            var listInfo = Singleton<CustomBase>.Instance.chaCtrl.lstCtrl.GetListInfo((ChaListDefine.CategoryNo)part.type, part.id);
                            if (listInfo == null)
                            {
            ...
            private static class CvsAccessoryChange_CopyAcs_Patches
            {
                ...
                {
                    var selSrc = __instance.selSrc;
                    var selDst = __instance.selDst;
                    var array = MessagePackSerializer.Serialize(_self.GetPart(selSrc));
                    _self.SetPart(selDst, MessagePackSerializer.Deserialize<ChaFileAccessory.PartsInfo>(array));
                    var dstPart = _self.GetPart(selDst);
                    if ((__instance.tglReverse).isOn)
                    {
                        var reverseParent = ChaAccessoryDefine.GetReverseParent(dstPart.parentKey);
                        if (string.Empty != reverseParent)
                        {
                    ...
                    var accessory = _self.GetCvsAccessory(selDst);
            ...
            private static class CvsAccessoryChange_CopyAcsCorrect_Patches
            {
                private static bool Prefix(CvsAccessoryChange __instance, int correctNo)
                {
                    var selSrc = __instance.selSrc;
                    var selDst = __instance.selDst;
                    var srcPart = _self.GetPart(selSrc);
                    var dstPart = _self.GetPart(selDst);
                    ChaFileAccessory.PartsInfo setPart = null;
                    if (selDst < 20)
            ...
            private static class CvsAccessoryChange_CopyAcsCorrectRevLR_Patches
            {
                private static bool Prefix(CvsAccessoryChange __instance, int correctNo)
                {
                    ...
                    for (var i = 0; i < 3; i++)
                    {
                        var vector = srcPart.addMove[correctNo, i];
                        if (i == 1)
                        {
                            vector.y += 180f;
                            if (vector.y >= 360f)
            ...
            private static class CvsAccessoryCopy_ChangeDstDD_Patches
            {
                private static void Postfix(CvsAccessoryCopy __instance)
                {
                    ...
                    var ddCoordeType = (__instance.ddCoordeType);
                    if (_self._charaMakerData.rawAccessoriesInfos.TryGetValue(ddCoordeType[0].value, out parts))
                    {
                        if (_self._additionalCharaMakerSlots.Count < parts.Count)
                            return;
                        for (var i = 0; i < _self._additionalCharaMakerSlots.Count; i++)
                        {
                            if (i < parts.Count)
                            {
                                var part = parts[i];
                                var listInfo = Singleton<CustomBase>.Instance.chaCtrl.lstCtrl.GetListInfo((ChaListDefine.CategoryNo)part.type, part.id);
                                _self._additionalCharaMakerSlots[i].copyDestinationText.text = listInfo == null ? "なし" : listInfo.Name;
                            }
                            else
                                _self._additionalCharaMakerSlots[i].copyDestinationText.text = "なし";
                        }
                    }
                    else
                    {
                    ...
                    var srcCount = 0;
                    if (_self._charaMakerData.rawAccessoriesInfos.TryGetValue(ddCoordeType[1].value, out parts))
                        srcCount = parts.Count;
                    var j = 0;
                    for (; j < srcCount; j++)
                        _self._additionalCharaMakerSlots[j].copySlotObject.SetActive(true);
                    for (; j < _self._additionalCharaMakerSlots.Count; ++j)
                        _self._additionalCharaMakerSlots[j].copySlotObject.SetActive(false);
                }
            }
                    ...
                    if (_self._charaMakerData.rawAccessoriesInfos.TryGetValue(ddCoordeType[1].value, out parts))
                    {
                        ...
                        for (var i = 0; i < _self._additionalCharaMakerSlots.Count; i++)
                        {
                            if (i < parts.Count)
                            {
                                var part = parts[i];
                                var listInfo = Singleton<CustomBase>.Instance.chaCtrl.lstCtrl.GetListInfo((ChaListDefine.CategoryNo)part.type, part.id);
                                _self._additionalCharaMakerSlots[i].copySourceText.text = listInfo == null ? "なし" : listInfo.Name;
                            }
                            else
                                _self._additionalCharaMakerSlots[i].copySourceText.text = "なし";
                        }
                    }
                    else
                    {
            ...
            private static class CvsAccessoryCopy_CopyAcs_Patches
            {
                private static void Prefix(CvsAccessoryCopy __instance)
                {
                    ...
                    if (_self._charaMakerData.rawAccessoriesInfos.TryGetValue(ddCoordeType[1].value, out srcParts) == false)
                    {
                        srcParts = new List<ChaFileAccessory.PartsInfo>();
                        _self._charaMakerData.rawAccessoriesInfos.Add(ddCoordeType[1].value, srcParts);
                    }
                    List<ChaFileAccessory.PartsInfo> dstParts;
                    if (_self._charaMakerData.rawAccessoriesInfos.TryGetValue(ddCoordeType[0].value, out dstParts) == false)
                    {
                        dstParts = new List<ChaFileAccessory.PartsInfo>();
                        _self._charaMakerData.rawAccessoriesInfos.Add(ddCoordeType[0].value, dstParts);
                    }
                    for (var i = 0; i < _self._additionalCharaMakerSlots.Count && i < srcParts.Count; i++)
                    {
                        var slot = _self._additionalCharaMakerSlots[i];
                        if (slot.copyToggle.isOn)
                        {
                            var array = MessagePackSerializer.Serialize(srcParts[i]);
                    ...
                    _self.ExecuteDelayed(_self.UpdateUI);
                }
            }
            ...
            private static class CvsAccessoryCopy_Start_Patches
            {
                private static void Postfix(CvsAccessoryCopy __instance)
                {
                    ...
                    {
                        foreach (var slot in _self._additionalCharaMakerSlots)
                        {
